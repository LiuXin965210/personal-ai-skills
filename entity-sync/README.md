...
对比 Java Entity 与 MySQL 表结构，识别字段、类型、注释、可空性、默认值和枚举说明差异，并生成同步建议或修改代码。

## 适用场景

- 检查 Entity 和数据库字段是否一致。
- 同步单个 Entity 或整个工程的实体类。
- 排查字段缺失、类型不匹配、注释不一致、可空性说明缺失。
- 基于 MySQL `IS_NULLABLE` 给 Java 字段备注补充“允许为空”或“不允许为空”。
- 检查枚举字段说明和废弃字段。
...
- 获取表结构时使用 `entity-sync/scripts/fetch_mysql_schema.py`，不要直接拼接 `mysql` 命令行。
- 操作生产数据库前需要明确确认。
...
- 查询表结构时使用 Python 脚本固定 `utf8mb4` 连接，避免中文注释乱码。
- Python 脚本依赖 `PyMySQL` 或 `mysql-connector-python` 之一。
