---
name: entity-sync
description: Java Entity 类与数据库表结构自动同步工具。检测字段差异、注释不一致、枚举值不匹配等问题。
---
...
从 JDBC URL 解析出数据库连接参数后，使用 `mysql` 命令行工具：

> ⚠️ **重要：必须指定字符集为 utf8mb4，否则中文注释会显示为乱码**

```bash
# 解析 JDBC URL: jdbc:mysql://host:port/database
# 提取: host, port, database

# 执行 SQL 查询表结构（注意：必须添加 --default-character-set=utf8mb4）
mysql -h {host} -P {port} -u {username} -p{password} {database} --default-character-set=utf8mb4 -e "
SELECT
    COLUMN_NAME,
    COLUMN_TYPE,
    IS_NULLABLE,
...
```bash
# 一次性获取多个表的结构（注意：必须添加 --default-character-set=utf8mb4）
mysql -h {host} -P {port} -u {username} -p{password} {database} --default-character-set=utf8mb4 -e "
SELECT
    TABLE_NAME,
    COLUMN_NAME,
    ...
    COLUMN_COMMENT
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_SCHEMA = '{database}' AND TABLE_NAME IN ('table1', 'table2', 'table3')
ORDER BY TABLE_NAME, ORDINAL_POSITION;
" 2>/dev/null
```

**安全提示：**
- **字符集**：必须添加 `--default-character-set=utf8mb4` 参数，否则中文注释会乱码
- 密码通过 `-p` 参数传递（注意 `-p` 后面直接跟密码，没有空格）
- 使用 `2>/dev/null` 隐藏密码警告信息
- 生产环境操作需要额外确认
...
| 字段名 | Entity 注释 | DB 注释 | 建议操作 |
|--------|-------------|---------|----------|
| `status` | `订单状态: 1-待支付 2-已支付` | `状态` | 更新 Entity 注释为 DB 注释 |

> **默认策略：以数据库注释为准更新 Entity 类注释**
> 原因：数据库注释通常是经过 DBA 审核的正式文档，且数据库注释修改需要执行 DDL，风险较高。
...
**当检测到注释不一致时，默认以数据库注释为准更新 Entity 类：**

1. **识别注释差异**：对比 Entity 字段注释和 DB 字段注释
2. **生成更新建议**：提供具体的 JavaDoc 注释修改内容
3. **批量更新**：对于多个注释不一致，生成批量修改方案
...
A: 使用替代方案：
```bash
# 使用 docker 中的 mysql 客户端（注意：必须添加字符集参数）
docker exec -i mysql-container mysql -u root -ppassword database --default-character-set=utf8mb4 -e "SQL"
...
A: 确保在 mysql 命令中添加 `--default-character-set=utf8mb4` 参数：
```bash
# 正确的命令格式
mysql -h host -P port -u user -ppassword database --default-character-set=utf8mb4 -e "SQL"

# 错误的命令格式（缺少字符集参数，会导致中文乱码）
mysql -h host -P port -u user -ppassword database -e "SQL"
```
